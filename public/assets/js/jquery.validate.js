...
jQuery.validator.setDefaults({
    ...
    errorPlacement: function (error, element) {
		if (element.closest('.form-group').hasClass('position-relative')) {
...
jQuery.validator.addMethod("validateName",function(fullname,element){
	...
	return this.optional(element) || /^[a-z ]{3,45}$/ig.test(fullname);
},"Họ và tên chứa các kí tự a-z, [space] (từ 3 đến 45 kí tự)");

jQuery.validator.addMethod("validateUsername",function(value,element){
	return this.optional(element) || /^[a-z0-9_-]{3,11}$/ig.test(value);
},"Tên đăng nhập chứa các kí tự a-z, 0-9, _, - (từ 3 đến 11 kí tự)!");

jQuery.validator.addMethod("validateTenMon",function(fullname,element){
	fullname = fullname.toLowerCase();
    fullname = fullname.replace(/à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ/g, "a");
    fullname = fullname.replace(/è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ/g, "e");
    fullname = fullname.replace(/ì|í|ị|ỉ|ĩ/g, "i");
    ...
    fullname = fullname.replace(/ỳ|ý|ỵ|ỷ|ỹ/g, "y");
    fullname = fullname.replace(/đ/g, "d");
	return this.optional(element) || /^[a-z0-9 _-]{2,}$/ig.test(fullname);
},"Tên môn chứa các kí tự a-z, 0-9, [space], _, - (từ 2 kí tự trở lên)!");
